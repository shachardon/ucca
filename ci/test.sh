...
sparse)
    ...
    ;;
dense)
    python parsing/parse.py -c dense --pos-tagger=stanford_english_bidirectional -WeLMCbs pickle/dev/*7* -t pickle/train/*7*
    ;;
mlp)
    python parsing/parse.py -c mlp --pos-tagger=stanford_english_bidirectional -WeLMCbs pickle/dev/*7* -t pickle/train/*7* --dynet-mem=3072
    ;;
bilstm)
    python parsing/parse.py -c bilstm --pos-tagger=stanford_english_bidirectional -WeLMCbs pickle/dev/*7* -t pickle/train/*7* --dynet-mem=3072 --lstmlayers=1 --lstmlayerdim=100
    ;;
tune)
