...
def get_annotation(terminal, attr):
    return attr(terminal.tok[attr.value])
...
def extract_aux(passage):
    for terminal in passage.layer(layer0.LAYER_ID):
...
def main(args):
    with open(args.outfile, "w", encoding="utf-8") as f:
        for passage in annotate_all(get_passages_with_progress_bar(args.passages), as_array=True):
...
if __name__ == "__main__":
    ...
    argparser.add_argument("-O", "--outfile", default="applied_rules.txt", help="output filename for report")
